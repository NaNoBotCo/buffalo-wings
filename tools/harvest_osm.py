#!/usr/bin/env python3
"""harvest_osm.py — every place OpenStreetMap ties to chicken wings, in all fifty states.
...
Two matchers, and every row says which one caught it:
  cuisine  cuisine=wings / chicken_wings / buffalo_wings, or chicken with wing in the name
  name     the word "wing" in the name of a restaurant, fast-food counter, bar or pub

A name match is weaker and this file says so: "Wing Lee" sells Cantonese food and
"Wingate Diner" sells breakfast. Rows tagged cuisine=chinese|thai|vietnamese|sushi are
dropped from the name matcher; the rest stay, labelled, for a human to read.
...
CUISINE_RE = "wings|chicken_wings|buffalo_wings"
NAME_RE = "[Ww]ing"
...
def classify(tags: dict) -> str | None:
    """'cuisine' (OSM says wings), 'name' (the name says wing), or None."""
    cu = (tags.get("cuisine") or "").lower()
    parts = [x.strip() for x in cu.replace(";", ",").split(",")]
    if any(p in ("wings", "chicken_wings", "buffalo_wings") for p in parts):
        return "cuisine"
    if "wing" in (tags.get("name") or "").lower():
        if any(p in NOT_WINGS for p in parts):
            return None
        return "name"
    return None
