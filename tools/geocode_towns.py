#!/usr/bin/env python3
"""geocode_towns.py — put the sauce makers on the map.

A sauce record's `profile` names the town and state printed on the label. This turns
...
Output: data/harvest/sauce-towns.json — a harvest file, never a record. build.py joins
it onto the sauce table, so a town nobody geocoded simply does not appear on the map.
...
def main(sleep=1.1) -> int:
    cache = jload(OUT) if OUT.exists() else {
        ...
        "note": "town centres for the towns printed on sauce labels; a point is the town, never the maker's door",
        "rows": {}}
    rows = cache["rows"]
    want = set()
    for r in load_nodes():
