...
Colour: one hue with position or lightness carrying the magnitude. The sequential sauce
...
def locator_svg(lat, lon, others: list, w=430, span=1.4, label=""):
    ...
    out.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="16" fill="var(--sauce)" opacity=".22"/>'
               f'<circle cx="{x:.1f}" cy="{y:.1f}" r="7" fill="var(--sauce)" stroke="var(--panel)" stroke-width="2.5"/>')
...
def where_svg(geo: dict, pts: list, w=430, title=""):
    ...
    for p in pts:
        ...
        out.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="4.5" fill="var(--sauce)" stroke="var(--panel)" stroke-width="1.5">'
