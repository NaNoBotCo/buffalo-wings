...
Outputs (all regenerated, never hand-edited):
  ...
  data/search/wings.thesaurus.json   mined from the records for search-core (synonymy only)
...
def mine_thesaurus(recs: list[dict]) -> int:
    ...
    for r in recs:
        ...
        if len(g) >= 2:
            groups.append(g)
    # spellings and nicknames a wing menu uses interchangeably
    groups += [
        ["buffalo wings", "buffalo wing", "hot wings", "wings", "wing"],
        ["barbecue", "barbeque", "bar-b-q", "bbq"],
        ["blue cheese", "bleu cheese", "blue cheese dressing"],
        ["drumette", "drummette", "drum", "drumstick end"],
        ["flat", "wingette", "flapper", "wingette flat"],
        ["lemon pepper", "lemon-pepper", "lemon peppa"],
        ["mumbo", "mambo", "mumbo sauce", "mambo sauce"],
        ["old bay", "chesapeake seasoning", "crab seasoning"],
        ["korean fried chicken", "kfc korean", "yangnyeom"],
        ["boneless wings", "boneless", "chicken nuggets"],
    ]
    SEARCH_DIR.mkdir(parents=True, exist_ok=True)
    jdump({"note": "mined by build.py from data/nodes names + aliases — synonymy only", "groups": groups}, SEARCH_DIR / "wings.thesaurus.json", indent=0)
    return len(groups)
...
def _norm(s: str) -> str:
    ...
    s = re.sub(r"\bwings\b", "wing", s)
    return re.sub(r"[^a-z0-9]+", " ", s).strip()


def sauce_table(recs: list[dict]) -> dict:
    """Every sauce record that carries a measured `profile`, flattened into one row each —
    the table behind /sauce/ and behind the builder's scale. Nothing is computed here that
    the record did not already state; a field the label did not carry stays null."""
    rows = []
    towns = (load_harvest("sauce-towns") or {}).get("rows", {})
    for r in recs:
        pf = r.get("profile") or {}
        if not pf:
            continue
        ...
        if not g and pf.get("town") and pf.get("state"):
            # the town printed on the label, geocoded by tools/geocode_towns.py — a point
            # is the town centre, never the maker's door, and the page says so
            g = towns.get(f'{pf["town"].strip()}, {pf["state"].strip()}') or {}
        rows.append({
            "id": r["id"], "name": r["names"]["name"], "url": f"sauce/{r['id']}/",
            "base": pf.get("base") or (r.get("facets") or {}).get("base"),
            "kind": pf.get("kind"), "maker": pf.get("maker", ""), "town": pf.get("town", ""), "state": pf.get("state", ""),
            "ingredients": pf.get("ingredients") or [],
            "pepper_rank": pf.get("pepper_rank"), "vinegar_rank": pf.get("vinegar_rank"), "fat_rank": pf.get("fat_rank"),
            "tomato_rank": pf.get("tomato_rank"), "sugar_rank": pf.get("sugar_rank"), "mustard_rank": pf.get("mustard_rank"),
            "soy_rank": pf.get("soy_rank"),
            "serving_tbsp": pf.get("serving_tbsp"), "sodium_mg_per_tbsp": pf.get("sodium_mg_per_tbsp"),
            "sugar_g_per_tbsp": pf.get("sugar_g_per_tbsp"), "scoville": pf.get("scoville"),
            "heat_claim": pf.get("heat_claim"), "note": pf.get("note", ""),
            "source": pf.get("source"), "source_url": pf.get("url"), "accessed": pf.get("accessed"),
            "lat": g.get("lat"), "lon": g.get("lon"),
        })
    rows.sort(key=lambda x: (x["base"] or "zz", x["name"].lower()))
    return {"built": time.strftime("%Y-%m-%d"), "count": len(rows),
            "note": ("One row per sauce record carrying a measured profile. Positions are 1-based ingredient order off "
                     "the label; 0 means the label never lists it; null means nobody here could read a label. Scoville "
                     "appears only where a maker or a lab published a number."),
            "sauces": rows}
...
def coverage(recs: list[dict], osm: dict | None, sources: dict) -> dict:
    ...
    return {
        ...
        "sources": len(sources),
        "not_yet": ["field observations (no record carries the field tier yet)", "menu prices with dates on most places",
                    "most cities beyond the ones written up", "photographs for most records", "Scoville numbers nobody publishes"],
        "tiers": TIER_LABEL,
    }
...
def main() -> int:
    ...
    jdump(sauce_table(recs), API / "sauces.json")
