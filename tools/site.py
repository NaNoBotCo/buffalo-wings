...
WING_CUTS = {k: v for k, _, v, _ in pages.WING_STYLES}
...
PATH_OF = {"style": "style", "sauce": "sauce", "dish": "dish", "fry": "fry", "place": "place", "person": "person", "org": "org", "event": "event", "term": "word", "art": "art", "story": "story"}
DIR_OF = {"style": "styles", "sauce": "sauces", "dish": "dishes", "fry": "kitchen", "place": "places", "person": "people", "org": "organizations", "event": "events", "term": "words", "art": "art", "story": "stories"}
...
CSS = """
:root{--bg:#f7f4ec;--panel:#fffdf6;--ink:#1d1a16;--mute:#6a6257;--line:#e3ddcd;--sauce:#cf4a1e;--smoke:#4a4540;--gold:#b08505;--lemon:#c2960a;--hot:#98301a;--blue:#2f5d8c;--celery:#5c8236;--focus:#1f5fa8;--chip:#efe9da;
  /* A wing joint letters its own board and hangs it over the fryer. Headings take a
     geometric sign face, labels take the fat caps of a bar special, and the reading
     text stays a plain humanist sans. All of it sits on the reader's machine already. */
  --display:Futura,"Futura Medium","Century Gothic","Avenir Next","Trebuchet MS",sans-serif;
...
@media (prefers-color-scheme: dark){:root:not([data-theme="light"]){--bg:#15130f;--panel:#1e1b16;--ink:#f3eee2;--mute:#b2a998;--line:#332e25;--sauce:#f2703f;--smoke:#cfc4b4;--gold:#dfb45a;--lemon:#e8c245;--hot:#f08a6b;--blue:#84aede;--celery:#9ec96f;--focus:#8ab4f8;--chip:#282219}}
:root[data-theme="dark"]{--bg:#15130f;--panel:#1e1b16;--ink:#f3eee2;--mute:#b2a998;--line:#332e25;--sauce:#f2703f;--smoke:#cfc4b4;--gold:#dfb45a;--lemon:#e8c245;--hot:#f08a6b;--blue:#84aede;--celery:#9ec96f;--focus:#8ab4f8;--chip:#282219}
...
a{color:var(--sauce);text-decoration-thickness:.07em;text-underline-offset:.16em}a:hover{color:var(--hot)}
...
header.top .brand{font-weight:700;text-decoration:none;color:var(--ink);letter-spacing:.01em}header.top .brand b{color:var(--sauce)}
...
.dir section{margin:0}.dir h2{margin:.2rem 0 .3rem;border:0;font-size:1.14rem;font-family:var(--display)}.dir h2 a{text-decoration:none;color:var(--ink)}.dir h2 a:hover{color:var(--sauce)}
...
.tier-cited{border-color:var(--focus)}.tier-harvested{border-color:var(--smoke)}.tier-tradition{border-color:var(--gold)}.tier-inference{border-style:dashed}.tier-field{border-color:var(--sauce)}
...
.kin a.card b{display:block;font-size:1.03rem;color:var(--sauce);font-family:var(--display);font-weight:700}.kin a.card small{display:block;font-size:.7rem;color:var(--gold);text-transform:uppercase;letter-spacing:.18em;font-family:var(--sign)}.kin a.card span{display:block;margin-top:.3rem;font-size:.92rem;color:var(--ink)}
...
.search{display:flex;gap:.5rem;margin:.6rem 0 1rem}.search input{flex:1;font:inherit;font-size:1.1rem;padding:.6rem .8rem;border:2px solid var(--line);border-radius:10px;background:var(--panel);color:var(--ink)}.search button{font:inherit;padding:.6rem 1rem;border-radius:10px;border:2px solid var(--sauce);background:var(--sauce);color:#fff;cursor:pointer}
...
.btn{display:inline-block;padding:.55rem 1.05rem;border-radius:999px;background:var(--sauce);color:#fff;text-decoration:none;font-weight:700;border:2px solid var(--sauce);font-family:var(--sign);font-size:.92rem;letter-spacing:.03em}.btn.ghost{background:transparent;color:var(--ink);border-color:var(--line)}.btn:hover{color:#fff;filter:brightness(1.08)}.btn.ghost:hover{color:var(--ink);border-color:var(--sauce)}
...
.whenbox .hrs.sold{color:var(--sauce);font-weight:600}
...
.tagrow a.tg:hover{border-color:var(--sauce)}
.tagrow .tg.own{border-color:var(--gold)}.tagrow .tg.welcome{border-color:#9b59b6}.tagrow .tg.pit{border-color:var(--sauce)}
...
{fleet.row_html("buffalo-wings")}
{fleet.support_html(self_id="buffalo-wings")}
{fleet.maker_html()}
</footer>
</body>
</html>
...
GROUP_LABEL.update({
    ...
    "main": "On the plate", "side": "Sides", "bread": "Bread", "sweet": "Sweets", "drink": "Drinks", "condiment": "Condiments",
    "cayenne-butter": "Cayenne and butter", "vinegar-pepper": "Vinegar and pepper", "dry-rub": "Dry rub",
    "citrus-pepper": "Citrus and pepper", "sweet-tomato": "Sweet and red", "soy-sweet": "Soy and sweet",
    "mustard": "Mustard", "dairy": "Dairy dips", "chile-crisp": "Chile crisp", "herb-garlic": "Herb and garlic", "other": "Other",
    "cut": "Cuts", "coating": "Coatings", "oil": "Oils", "fryer": "Fryers", "method": "Methods", "prep": "Preparation",
    "tool": "Tools", "heat": "Heat", "smoke": "Smoke",
    "sign": "Signs", "neon": "Neon", "mascot": "Mascots", "mural": "Murals", "print": "Prints", "poster": "Posters", "photograph": "Photographs",
    "essay": "Essays", "map": "Maps explained", "quiz": "Games", "resources": "Where it came from",
    "cook": "Cooks", "founder": "Founders", "family": "Families", "writer": "Writers", "scholar": "Scholars",
    "organizer": "Organizers", "chef": "Chefs", "sauce-maker": "Sauce makers", "competitor": "Eaters and competitors",
...
def map_svg(places: list[dict], recs_by_id: dict, depth: int, width=760) -> str:
    """Inline SVG: fifty states in Albers equal-area, every wing place a dot; places
    written up here are sauce-coloured and link to their page, OpenStreetMap rows are
    ...
    labels = [(-78.87, 42.89, "Buffalo", "end", -12), (-77.61, 43.16, "Rochester", "start", -3),
              (-87.63, 41.84, "Chicago · mild sauce", "end", -9), (-76.61, 39.29, "Baltimore · Old Bay", "start", -13),
              (-77.03, 38.90, "D.C. · mumbo", "start", 13), (-90.05, 35.15, "Memphis · dry rub", "end", -7),
              (-86.78, 36.16, "Nashville · hot", "start", -7), (-86.80, 33.52, "Alabama · white sauce", "end", 5),
              (-84.39, 33.75, "Atlanta · lemon pepper", "start", 17)]
    lab = []
    ...
    return (f'<svg viewBox="0 0 {w} {h:.0f}" xmlns="http://www.w3.org/2000/svg" role="img" '
            f'aria-label="Map of the United States with every wing place as a dot">'
...
def node_jsonld(r: dict) -> list:
    ...
    if r["type"] == "place":
        a = r.get("address") or {}
        g = r.get("geo") or {}
        base.update({"@type": "Restaurant", "servesCuisine": "Chicken wings"})
        if a:
...
def kin_block(r: dict, by_id: dict, depth: int) -> str:
    ...
    for k in r.get("kin_out", []):
        ...
        cards.append(f'<a class="card" href="{rel(depth)}{url_of(t)}index.html"><small>{E(DIR_OF[t["type"]] if t["type"] != "fry" else "the kitchen")}</small><b>{E(t["names"]["name"])}</b><span>{E(k["as"])}</span>'
...
def node_page(r: dict, by_id: dict, sources: dict) -> str:
    ...
    if r.get("primary_image"):
        ...
        cuts = WING_CUTS.get(r["id"], set())
        hero = (f'<figure class="hero-draw">{pages.wing_svg(cuts, label=False, ident=r["id"])}'
                f'<figcaption>{E({3: "The whole wing, tip and all.", 2: "Cut in two; the tip goes to the stockpot."}.get(len(cuts), "Which pieces this style serves."))}</figcaption></figure>')
    ...
    if pf:
        ...
        prows = [("what was read", read or "no label found"),
                 ...
                 ("tomato", rank("tomato_rank")), ("sugar", rank("sugar_rank")), ("soy sauce", rank("soy_rank")),
        ...
        prows.append(("Scoville", f'{pf["scoville"]:,.0f} SHU' if pf.get("scoville") is not None
                      else '<span class="mute">nobody publishes one for this bottle</span>'))
...
def front_page(recs: list[dict], by_id: dict, places: dict, types: dict, coverage: dict) -> str:
    ...
    jl = [{"@context": "https://schema.org", "@type": "Dataset", "name": SITE_NAME, "description": "A structured directory of the chicken wing in the United States: regional styles, sauces and dips, dishes, kitchen practice, places, people, organizations, events and vocabulary, one JSON record per node with per-field provenance.",
           "url": SITE_URL + "/", "license": DATA_LICENSE, "creator": AUTHOR, "publisher": fleet.publisher_ld(), "includedInDataCatalog": fleet.catalog_ld(), "isAccessibleForFree": True, "keywords": ["buffalo wings", "chicken wings", "hot wings", "wing sauce", "lemon pepper", "blue cheese", "ranch", "Buffalo, New York"],
    ...
    return page(f"{SITE_NAME} — {TAGLINE}", body, depth, "A directory of the American chicken wing: styles, sauces, dips, dishes, kitchen practice, places, people, events and words, each with its sources.", jl, SITE_URL + "/",
                card="index", og_alt="Wing Country: a directory of a living argument", share_title=SITE_NAME)
...
def manifest() -> str:
    return json.dumps({"name": SITE_NAME, "short_name": "Wings", "start_url": "./index.html", "display": "standalone", "background_color": "#f7f4ec", "theme_color": "#cf4a1e",
                       "description": TAGLINE, "icons": [{"src": "icon.svg", "sizes": "any", "type": "image/svg+xml"}]}, indent=1)


def icon_svg() -> str:
    # the wing itself: three capsules in a shallow Z, the same drawing the site uses at size
    return ('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="12" fill="#cf4a1e"/>'
            '<g stroke="#f7f4ec" fill="none" stroke-linecap="round">'
            '<line x1="14" y1="46" x2="30" y2="35" stroke-width="13"/>'
            '<line x1="30" y1="35" x2="47" y2="39" stroke-width="10"/>'
            '<line x1="47" y1="39" x2="55" y2="45" stroke-width="5"/></g>'
            '<circle cx="30" cy="35" r="3.4" fill="#cf4a1e"/><circle cx="47" cy="39" r="2.8" fill="#cf4a1e"/></svg>')


def llms_txt(recs: list[dict], cov: dict) -> str:
    lines = [f"# {SITE_NAME}", "",
             "> A structured directory of the chicken wing in the United States: regional styles, sauces and dips, dishes, kitchen practice, places, people, organizations, events and vocabulary. One JSON record per node; every field carries a provenance tier (cited / harvested / tradition / inference / field); records say what their neighbours are to them in both directions.",
...
def ai_txt() -> str:
    return (f"# {SITE_NAME} — {SITE_URL}/\n"
            ...
            "Attribution: Wing Country, " + SITE_URL + "/\n\n"
...
def main() -> int:
    ...
    sauces = jload(API / "sauces.json") if (API / "sauces.json").exists() else {}
    geo = jload(GEO / "states.json")
    global GEO_CACHE
    GEO_CACHE = geo
    for name, html_text in (("near", pages.near_page(page, places, recs, tagvocab, SITE_URL)),
                            ("sauce", pages.sauce_page(page, sauces, recs, geo, SITE_URL)),
    ...
    fleet.decorate(SITE, "buffalo-wings")
    wk = SITE / ".well-known"
